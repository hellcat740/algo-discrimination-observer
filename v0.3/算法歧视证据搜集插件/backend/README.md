...
│   └── static/admin.html  # 数据库查看前端（原生 JS，无框架）
├── requirements.txt
...
- `main.py` 已挂载 `CORSMiddleware`：`allow_origins=["*"]`、`allow_methods=["*"]`、
  ...
  与任意来源页面可直调后端。
- ⚠️ 这是**本地演示**配置；生产部署应收敛 `allow_origins` 到具体来源（插件 ID 对应的
...
| POST | `/api/admin/seed-demo` | 灌入演示数据，body `{"clear": true/false}`（⚠️ 仅供开发调试，正式环境勿用） |
| POST | `/api/admin/clear-demo` | 清空 DEMO- 前缀演示数据（⚠️ 仅供开发调试） |
...
- 价格接受「分」(int) 或「元」(float / 含小数的字符串)，元自动 ×100 四舍五入转分并记 warning；
  金额范围 `1 ~ 1,000,000` 分，越界返回 422。
- `platform_code` 白名单：`meituan, taobao, tmall, jd, pinduoduo, yangkeduo, eleme, dianping, ctrip, didi`，不在名单返回 422 并列出合法值。
- 字符串超长自动截断并记 warning（user_agent ≤500、product_name ≤300、product_url ≤1000、其余 ≤200）。
...
4. **判定规则**（逐条体现在 `reason_code`）：
   ...
   - `REGRESSION_SIGNIFICANT`：回归核心变量 p < 0.05；回归不适用时记
     `REGRESSION_NOT_APPLICABLE`（不计入否决）
...
```json
{
  ...
  "groups": [
    ...
    },
    {
      ...
      "min_cents": 9284.0,
      "max_cents": 9778.0,
      "prices_cents": [9284.0, 9326.0, 9346.0, 9356.0, 9361.0, 9401.0, 9401.0, 9445.0, 9467.0, 9479.0, 9483.0, 9496.0, 9502.0, 9512.0, 9541.0, 9588.0, 9608.0, 9629.0, 9634.0, 9639.0, 9649.0, 9655.0, 9656.0, 9667.0, 9674.0, 9692.0, 9703.0, 9716.0, 9723.0, 9778.0]
    }
  ],
  "mann_whitney": {
    "u_stat": 0.0,
