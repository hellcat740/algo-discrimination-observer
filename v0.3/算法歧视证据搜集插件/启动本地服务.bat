...
rem ---------- 0. 系统完整性自检：backend 目录缺失时自动修复 ----------
if not exist "%~dp0backend\" (
  echo 未检测到 backend 目录，正在检查上一级目录...
  if exist "%~dp0..\backend\" (
    echo 在上一级目录找到 backend 文件夹，正在复制到本目录...
    xcopy "%~dp0..\backend" "%~dp0backend\" /E /I /H /Y /Q >nul
  ) else (
    for %%f in ("%~dp0..\backend*.zip") do (
      echo 在上一级目录找到压缩包 %%~nxf，正在解压到本目录...
      powershell -NoProfile -Command "Expand-Archive -LiteralPath '%%~ff' -DestinationPath '%~dp0' -Force"
      goto :backend_after_restore
    )
    echo 【错误】上一级目录中既没有 backend 文件夹，也没有 backend 压缩包，无法自动修复。
    pause
    exit /b 1
  )
)
:backend_after_restore
if not exist "%~dp0backend\" (
  echo 【错误】backend 目录恢复失败，请手动检查上一级目录的内容。
  pause
  exit /b 1
)
