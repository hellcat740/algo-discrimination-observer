...
| 方法 | 路径 | 说明 |
| --- | --- | --- |
| GET | `/api/admin/users` | 匿名用户列表（anonymous_id、首次/最近活跃时间、观测数），按最近活跃倒序 |
| DELETE | `/api/admin/users/{anonymous_id}` | 删除该用户全部观测及 users 建档，返回 `{deleted_observations, deleted_user}`；不存在 404 |

### 数据通路（本机 ⇄ 服务器）

| 方法 | 路径 | 说明 |
| --- | --- | --- |
| POST | `/api/local/sync/push` | 立即上行本机待同步观测到 `SYNC_SERVER_URL`（按 observation_id 幂等，可重试） |
| GET | `/api/local/sync/status` | 同步状态：待同步/已同步/留存拉取数量与服务器地址（未配置时 `server_url` 为 null） |
| POST | `/api/local/analysis/pull` | 拉取服务器同商品全池数据与本机合并分析，body `{"product_id":"…","keep":false}`；`keep=false` 分析完即删，`keep=true` 以 `origin='pulled'` 留存 |

> 配置了 `SYNC_SERVER_URL` 环境变量后，启动时自动开启后台线程按 `SYNC_INTERVAL_SEC`
> （默认 300 秒）周期上行；未配置则为单机模式。Vercel 环境（`VERCEL=1`）自动禁用自动上行，
> 防止公网服务器自我同步回环。控制台 UI：总览页「服务器同步」卡片（状态 + 立即上行）、
> 歧视分析页「拉取服务器同商品数据合并比对 / 保留拉取数据到本地」复选框、
> 观测数据页「用户 / 来源」列与「👥 用户管理」抽屉。
...
| 方法 | 路径 | 说明 |
| --- | --- | --- |
| GET | `/` | 门户首页：用户一键登录 / 管理员登录 / 插件下载 |
| GET | `/me` | 用户个人看板（X-User-Id 免密） |
| GET | `/admin-login` | 管理员登录页 |
| GET | `/console` | 管理控制台（单页应用）：总览 / 观测数据 / 歧视分析 / 关于，需管理员密钥 |
| GET | `/download` | 插件下载页；`/download/plugin.zip` 直接下载 |
