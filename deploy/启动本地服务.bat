...
cd /d "%~dp0repo\backend"
if errorlevel 1 (
  echo 【错误】未找到 repo\backend 目录，请确认本脚本位于 deploy 根目录。
  pause
  exit /b 1
)
...
rem ---------- 3. 公网同步目标 ----------
rem 本机采集的数据会自动上行到该服务器；未手动指定时默认指向官方演示服务器。
if not defined SYNC_SERVER_URL set "SYNC_SERVER_URL=https://algo-observer-server.vercel.app"
echo 数据同步目标：%SYNC_SERVER_URL%

rem ---------- 4. 端口检测：已在运行则不重复启动 ----------
